...
const Signup = () => {
    ...
    const SignInSchema = Yup.object().shape({
        ...
        confirmPassword: Yup.string().required("Confirmer votre mot de passe").oneOf([Yup.ref('password'), null], 'Mots de passe ne correspond pas. Veuillez réessayer')    
      })
      ...
      function validatePassword(value: string){
        ...
        if (!value) {
          ...
          error = 'Le mot de passe doit comporter au moins 8 caractères, au moins un caractère spécial et au moins un caractère numérique.';
        }
        return error;
      }
            ...
            {({  errors, touched }) => (
              <Form >
                ...
                validate={validateLastName}
                 />
                <Box height="1rem">{errors.lastName && touched.lastName ? <Box fontSize="12" color='red'>{errors.lastName}</Box> : null}</Box>
                </VStack>
                ...
                validate={validateFirstName}
                 />
                <Box mb="2rem" height="1rem">{errors.firstName && touched.firstName ? <Box fontSize="12" color='red'>{errors.firstName}</Box> : null}</Box>
                </VStack>
                ...
                name="username"
                type="text" />
                {errors.username && touched.username ? <Box fontSize="12" color='red'>{errors.username}</Box> : null}

                <FormLabel  mt= "1rem" htmlFor="email">Email Address</FormLabel>
                <Field 
                as={Input}
                id="email"
                name="email"
                ...
                validate={validateEmail}
                 ...
                 <FormLabel mt= "1rem" htmlFor='confirmPassword'>Confirmer Mot de passe</FormLabel>
                 <Field
                 as={Input}
                 id="confirmPassword"
                 ...
                 type="confirmPassword"
                  />
                 {errors.confirmPassword && touched.confirmPassword ? <Box fontSize="12" color='red'>{errors.confirmPassword}</Box> : null}
